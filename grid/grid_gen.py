...
def inputFlowChannels(minChannel, maxChannel, minMux, minCellTrap, f):
	f.write("CHANNEL c" + str(minChannel) + " from p1 3 to m1 " + str(minMux) + " channelWidth=100;\n")
	for i in range (minChannel + 1, maxChannel+1):
		minMux=minMux+1
		f.write("CHANNEL c" + str(i) + " from m1 " + str(minMux) + " to ct" + str(minCellTrap) + " 1 channelWidth=100;\n")
		minCellTrap=minCellTrap + 1

def cellTrapGridH(minChannel, maxChannel, minCellTrap, f):
	for i in range(minChannel, maxChannel):
		f.write("CHANNEL c" + str(i) + " from ct" + str(minCellTrap) + " 2 to ct" + str(minCellTrap+1) + " 4 channelWidth=100;\n")
		minCellTrap = minCellTrap + 1

def cellTrapGridV(minChannel, maxChannel, minCT1, minCT2, f):
	for i in range(minChannel, maxChannel):
		f.write("CHANNEL c" + str(i) + " from ct" + str(minCT1) + " 3 to ct" + str(minCT2) + " 1 channelWidth=100;\n")
		minCT1 = minCT1 + 1
		minCT2 = minCT2 + 1

def outputFlowChannels(minChannel, maxChannel, minMux, minCellTrap, f):		
	for i in range (minChannel, maxChannel-1):
		f.write("CHANNEL c" + str(i) + " from ct" + str(minCellTrap) + " 3 to m2 " + str(minMux)+ " channelWidth=100;\n")
		minCellTrap=minCellTrap + 1
		minMux=minMux+1
	f.write("CHANNEL c" + str(maxChannel-1) + " from m2 " +  str(minMux) + " to p2 1 channelWidth=100;\n")

def hValves(minValve, maxValve, minChannel, f):
	for i in range(minValve, maxValve):
		f.write("VALVE v" + str(i) + " on c" + str(minChannel) + " channelWidth=300 l=100;\n")
		minChannel = minChannel + 1

def vValves(minValve, maxValve, minChannel, f):
	for i in range(minValve, maxValve):
		f.write("VALVE v" + str(i) + " on c" + str(minChannel) + " channelWidth=100 l=300;\n")
		minChannel = minChannel + 1
...
def controlChannels(minChannel, maxChannel, minValve, port, f):
	for i in range(minChannel, maxChannel):
		f.write("CHANNEL cc" + str(i) + " from v" + str(minValve) + " 2 to v" + str(minValve+1) + " 4 channelWidth=50;\n")
		minValve = minValve + 1
	f.write("CHANNEL cc" + str(i+1) + " from v" + str(minValve) + " 2 to cpb2 " + str(port) + " channelWidth=50;\n")
...
f.write("V BANK cpb2 of " + str(grid_size-1) + " PORT portRadius=100  spacing=1500 ;\n")
newline(f)
for i in range(0, k):
