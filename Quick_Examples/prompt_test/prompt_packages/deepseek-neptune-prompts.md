# Neptune Prompt Package — DeepSeek

> **Why Markdown?** DeepSeek chat does not accept `.zip` uploads.
...
- Optimized for code: keep identifiers consistent; minimize chatter.
- Long system prompts benefit from API prefix caching—this block may be reused verbatim across requests.
...
<!-- source: deepseek/lfr2en_system.txt -->

```text
Explain LFR source in technical English.

Rules:
- Do not invent ports, behavior, controls, mappings, or constraints not in the file.
...
<!-- source: deepseek/README.txt -->

```text
Neptune DeepSeek prompt pack

Export format:
- Neptune GUI exports this pack as a single .md file (not .zip).
- DeepSeek chat does not accept .zip uploads.

Setup (one time):
1. Download deepseek-neptune-prompts.md from the Editor LLM prompts panel,
   OR open en2lfr_system.txt from this folder.
2. Paste the English→LFR system section into DeepSeek chat / API system prompt.
...
<!-- source: deepseek/en2lfr_user_template.txt -->
...
<!-- source: deepseek/lfr2en_user_template.txt -->
